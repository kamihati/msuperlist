...
class NewListTest(TestCase):
    ...
    def test_saving_a_POST_request(self):
        self.client.post('/lists/new', data=dict(item_text='A new list item'))
        self.assertEqual(Item.objects.count(), 1)
        new_item = Item.objects.first()
        self.assertEqual(new_item.text, 'A new list item')

    def test_redirects_after_POST(self):
        response = self.client.post('/lists/new', data=dict(item_text='A new list item'))
...
class NewItemTest(TestCase):
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post(
            ...
            data=dict(item_text='A new item for and existing list'))

        self.assertEqual(Item.objects.count(), 1)
        new_item = Item.objects.first()
        self.assertEqual(new_item.text, 'A new item for and existing list')
